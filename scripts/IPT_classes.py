...
class Purchase_Plan(object):
    ...
    def __init__(self, title, demand_plan, returns_plan, print_plan, ss_plan, order_n_months_supply = 9, inv_0 = 0):
        ...
        #self.demand_plan = demand_plan # shorter variable names. self.d_plan ? 
        self.d_plan = demand_plan
        # reference months, forecast, etc. on the fly/as needed 
        ## months = self.demand_plan.months
        # self.months = demand_plan.months # just access this through self.demand_plan
        # self.forecast = demand_plan.forecast
        ...
        if ss_plan.reorder_point == None:
            self.reorder_point = [0] * len(self.d_plan.months)
        else:
        ...
        temp = self.determine_plan(self.d_plan, self.reorder_point, self.inv_0, self.returns, self.calc_order_qty)
        self.POD_orders, self.digital_orders, self.orders, self.offshore_orders, self.starting_inventory, \
                self.ending_inventory, self.average_inventory, = temp
        ...
        self.POD_FMC, self.digital_FMC, self.conv_FMC, self.offshore_FMC, self.POD_VMC,  \
                ...
                self.carry_stg_cost, self.lost_sales_expected = self.calc_costs(self.d_plan.forecast, self.d_plan.sd_forecast, self.returns, 
                    self.POD_orders, self.digital_orders, self.orders, self.offshore_orders, self.starting_inventory , self.average_inventory,)

       # temp = self.calc_costs(self.d_plan.forecast, self.d_plan.sd_forecast, self.returns, self.POD_orders, self.digital_orders,
       #                      self.orders, self.offshore_orders, self.starting_inventory, self.average_inventory,)
       # self.FMC = temp[0:4] #POD, Digital, Conventional, Offshore
       # self.VMC = temp[5:8]
       # self.umc, self.carry_stg_cost, self.lost_sales_expected = temp[9:11] # off by one?
    ...
    def determine_plan(self, d_plan, rop, inv_0, returns, calc_order_qty):
        ...
        for i, fcst in enumerate(d_plan.forecast):
            ...
            if trial_ending_inventory < rop[i]: # place order # replace with "get reorder point function"

                POD_orders[i], digital_orders[i], orders[i], offshore_orders[i] = calc_order_qty(i, d_plan.forecast, returns)

            inv_delta = - d_plan.forecast[i] + returns[i] + orders[i] + POD_orders[i] + digital_orders[i] + offshore_orders[i]
            end_inv.append(start_inv[i] + inv_delta)
